...
if __name__ == '__main__':
    ...
    def features_for(image):
        max_features    = 64 # good for non-thin image
        #max_features    = 48
        features        = cv2.goodFeaturesToTrack(image, max_features, 0.02, 3)
    ...
    for i in xrange(100):
        ...
        for c in xrange(10):
            ...
            # thinning
            #thinner = ZhangSuen()
            #image = thinner.thin(image)
    
            features.append(features_for(image))
            labels.append(c)
    
    samples = np.array(features, dtype=np.float32)
